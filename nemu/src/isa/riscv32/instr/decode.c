...
static def_DHelper(S) {
    ...
    word_t imm = (s->isa.instr.s.simm11_5 << 5) | s->isa.instr.s.imm4_0;
...
static def_DHelper(B) {
    ...
    word_t imm = (s->isa.instr.b.imm_12 << 12) | (s->isa.instr.b.imm_11 << 11) | (s->isa.instr.b.imm_10_5 << 5) |
                   (s->isa.instr.b.imm_4_1 << 1);
    decode_op_i(s, id_dest, imm, false);
}
...
static def_DHelper(J) {
    ...
    word_t imm = (s->isa.instr.j.imm_20 << 20) | (s->isa.instr.j.imm_19_12 << 12) | (s->isa.instr.j.imm_11 << 11) |
                   (s->isa.instr.j.imm_10_1 << 1);
    decode_op_i(s, id_src1, imm, false);
...
//def_THelper(sync) {
//    def_INSTR_TAB("0000??? ????? 00000 000 00000 ????? ??", fence);
//    def_INSTR_TAB("0000000 00000 00000 001 00000 ????? ??", fencei);
//    return EXEC_ID_inv;
//}

//def_THelper(call) {
//    def_INSTR_TAB("??????? ????? ????? 000 ????? ????? ??", ecall);
//    def_INSTR_TAB("??????? ????? ????? 000 ????? ????? ??", ebreak);
//    def_INSTR_TAB("??????? ????? ????? 000 ????? ????? ??", csrrw);
//    def_INSTR_TAB("??????? ????? ????? 000 ????? ????? ??", csrrs);
//    def_INSTR_TAB("??????? ????? ????? 000 ????? ????? ??", csrrc);
//    def_INSTR_TAB("??????? ????? ????? 000 ????? ????? ??", csrrwi);
//    def_INSTR_TAB("??????? ????? ????? 000 ????? ????? ??", cssrrsi);
//    def_INSTR_TAB("??????? ????? ????? 000 ????? ????? ??", csrrci);
//    return EXEC_ID_inv;
//}
...
def_THelper(compute_r) {
    ...
    def_INSTR_TAB("0000001 ????? ????? 010 ????? ????? ??", mulh);
    def_INSTR_TAB("0000001 ????? ????? 011 ????? ????? ??", mulhsu);
...
def_THelper(m_i) {
    def_INSTR_IDTAB("0000000 00000 00000 000 00000 ????? ??", I, ecall);
    def_INSTR_IDTAB("0011000 00010 00000 000 00000 ????? ??", I, mret);
...
def_THelper(main) {
    ...
    def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11100 11", I, m_i);
    def_INSTR_TAB  ("??????? ????? ????? ??? ????? 11010 11", nemu_trap);
    return table_inv(s);
};

//    def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00011 11", I, sync);
//    def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11100 11", I, call);
